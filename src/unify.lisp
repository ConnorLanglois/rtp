...
;;;;;
;;;;;    What is missing is any way to instantiate a FOPC expression.  You'll
;;;;;    need a function that does something like the following:
;;;;;        (instantiate '(or (not (dog ?x)) (cat ?x)) '((y 6) (x Fluffy)))
;;;;;           --> (or (not (dog Fluffy)) (cat Fluffy))
;;;;;
;;;;; Author: Roy Turner
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;; Function: unify-axioms
;;; Arguments:
;;; 	- axiom1: an axiom
;;; 	- axiom2: an axiom
;;; 	- bindings (optional): bindings list of the axioms
;;; Returns: whether the two axioms unify and the bindings list resulting
;;; 	or the old bindings list
;;; Description:
;;; 	Attempts to unify two axioms.
;;; 	As "unify" function only unifies two predicates, this function is needed
;;; 	to unify two axioms, which may each consist of one or predicates.
;;; 	Loops through each axiom quadratically, unifying each predicate together
;;; 	and merging binding lists.
;;; Author: Connor Langlois <connor.langlois@maine.edu>
;;; Created: 3/10/2019
;;; Modifications: none
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

(defun unify-axioms (axiom1 axiom2 &optional bindings)
	(let (matched?)
		(loop for p1 in (to-terms (un-disjunct axiom2))
			do (loop for p2 in (to-terms (un-disjunct axiom1))
				do (multiple-value-bind (new-matched? new-bindings) (unify p1 p2 bindings)
					(setf matched? (or matched? new-matched?))
					(setf bindings (union bindings new-bindings)))))
		(values matched? bindings)))
...
;;;
;;; * Added functionality to strip "not" from predicate
;;;
;;;]
...
(defun unify (p1 p2 &optional bindings)
	(cond
		...
		((and (listp p1) (not? p1))
			(unify (extract-p p1) p2))
		((and (listp p2) (not? p2))
			(unify p1 (extract-p p2)))
...
(defun unify-elements (p1 p2 bindings)
	(let (blist matched?)
		...
		(cond
		 ((null matched?)
			(values nil bindings))
		 ((multiple-value-setq (matched? blist) (unify (rest p1) (rest p2) blist))
